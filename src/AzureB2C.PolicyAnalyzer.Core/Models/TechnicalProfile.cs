...
namespace AzureB2C.PolicyAnalyzer.Core.Models
{
    public class TechnicalProfile : PolicyItem
    {
        ...
        public string ProtocolName => XmlNode.Element(PolicyItem.ns + "Protocol").Attribute("Name").Value;
        public ClaimType SubjectNamingInfo => References.GetReference<ClaimType>(XmlNode.Element(PolicyItem.ns + "SubjectNamingInfo").Attribute("ClaimType").Value, this.Policy);
